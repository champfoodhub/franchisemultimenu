 ...
 * Run with: npx tsx tests/manual/product-schedules.ts
 */

import { storage, seed } from '../../server/mock-storage.js';
...
async function runTests() {
  ...
  try {
    ...
    const scheduleIds = schedules.slice(0, Math.min(2, schedules.length)).map((s: any) => s.id);
    ...
    const schedule = schedules.find((s: any) => s.id === schedules[0].id);
    ...
    for (const p of products.slice(0, 3)) {
      const pSchedules = schedules.slice(0, Math.min(2, schedules.length)).map((s: any) => s.id);
    ...
    const timeSlotSchedules = schedules.filter((s: any) => s.type === 'TIME_SLOT');
    const seasonalSchedules = schedules.filter((s: any) => s.type === 'SEASONAL');
